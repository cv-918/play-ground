...
async function main() {
  ...
  const completionReportId = runnerStart.report_ids?.completion_report_id;
  const runnerRunId = runnerStart.runner_run_id;
  ...
  if (runnerStart.stop_reason !== "completion_review_required") {
    return fail(reportRoot, smokeRoot, "completion_gate", { taskId, runnerStart });
  }
  if (!completionReportId || !runnerRunId) {
    return fail(reportRoot, smokeRoot, "runner_artifacts", { taskId, runnerStart });
  }
  ...
  const report = {
    ...
    runner_start_stop_reason: runnerStart.stop_reason,
