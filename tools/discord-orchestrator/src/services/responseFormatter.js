...
export function formatDocs() {
  return [
    ...
    "",
    "**사용자가 먼저 볼 문서**",
    "- `_Docs/AIWorkflow/Guide/AIWorkflow_User_Guide_KR.html` - 현재 정규 워크플로우를 브라우저에서 보는 완성형 사용자 가이드",
    "- `_Docs/AIWorkflow/FinalBlueprint/WF_Discord_Command_Quick_Reference_KR.md` - Runner가 멈췄을 때 다음 명령을 찾는 치트시트",
    "- `_Docs/AIWorkflow/FinalBlueprint/WF_Human_Director_Operation_Guide_KR.md` - Human Director 운영 배경과 companion 가이드",
    "",
    "**AI/에이전트 운영 규칙**",
    "- `AGENTS.md` - 이 repo에서 AI가 지켜야 하는 최상위 작업 규칙",
    "- `.github/copilot-instructions.md` - Copilot/Agent용 repository 작업 지침",
    "- `_Docs/AIWorkflow/README.md` - AIWorkflow 문서 색인과 현재 정규 흐름",
    "",
    "**현재 작업 상태 파일**",
    "- `_Docs/AIWorkflow/ProjectStatus.md` - 전체 워크플로우 진행 상태",
    "- `_Docs/AIWorkflow/Backlog.md` - 접수된 작업 목록",
    "- `_Docs/AIWorkflow/ActiveTask.md` - 현재 선택된 작업과 승인 상태",
    "",
    "**상태/프로필 모델**",
    "- `_Docs/AIWorkflow/Task_State_Model.md` - Backlog/ActiveTask/task done 상태 모델",
    "- `_Docs/AIWorkflow/Project_Profile_Schema.md` - 프로젝트 프로필 스키마",
    "- `_Docs/AIWorkflow/Active_Project_Selector.md` - 현재 활성 프로젝트 선택 규칙",
    "",
    "**Discord 명령 설계 문서**",
    "- `_Docs/AIWorkflow/Discord_Task_Management_Commands.md` - task create/set-active/done 등 작업 관리 명령",
    "- `_Docs/AIWorkflow/Discord_Task_Status_Commands.md` - 승인, 상태 note, task 상태 확인 명령",
    "- `_Docs/AIWorkflow/Discord_Safe_Script_Execution_Commands.md` - allowlist 기반 안전 스크립트 실행 명령",
    "- `_Docs/AIWorkflow/Discord_Codex_Task_Routing_Commands.md` - legacy/manual Codex routing 명령",
    "- `_Docs/AIWorkflow/Discord_Goal_Task_Routing_Commands.md` - goal request 생성 명령",
    "- `_Docs/AIWorkflow/Discord_Role_Recommendation_Command.md` - role recommendation 명령",
  ].join("\n");
}
