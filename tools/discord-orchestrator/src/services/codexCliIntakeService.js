...
function buildPrompt(input) {
  ...
  return [
    ...
    "- Safe GAME/build validation exception: if the request explicitly says no source/data/schema/runtime/document changes and asks only to validate Visual Studio/MSBuild/Debug x64 build, classify as category VAL, kind validation, priority P2, suggested_risk low.",
    "- For that safe build validation exception, do not ask which runner profile or command_id to use. The deterministic route is profile=build, executor=local_cli, command_id=debug_visual_studio_build.",
    "- Safe GAME data loader/readability validation exception: if the request explicitly says no source/data/schema/runtime/document changes and asks only for GameDataLoader expected-file, JSON shape, ID/reference, or readability validation, classify as category VAL, kind validation, priority P2, suggested_risk low.",
    "- For that safe data-readability validation exception, do not ask which runner profile or command_id to use. The deterministic route is profile=validation, executor=local_cli, command_id=game_data_loader_readability.",
    "- Keep clarifying questions only for validation work that asks for an unknown command or evidence type that is not already covered by those deterministic routes.",
    "",
