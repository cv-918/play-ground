...
struct _Color
{
	...
	void SetAlpha(_float _alpha)
	{
		...
		const auto alpha = static_cast<_uint>(std::round(_alpha * 255.f));
...
namespace Path
{
	...
	const std::wstring Ui = Texture + L"UI/";
	const std::wstring Buttons = Ui + L"Buttons/";
	const std::wstring PopUps = Ui + L"PopUps/";
