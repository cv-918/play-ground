...
public:
	...
	_bool Pause() const { return pause; }
	void Pause(const _bool _pause);

	_uint CoinCount() const { return coin_count; }
	void AddCoin(_uint _count) { coin_count += _count; }

private:
	GameObjectBase* player_;

	_bool pause = false; // 게임 전체 일시정지 여부를 나타내는 플래그. 필요에 따라 게임 전체의 업데이트와 렌더링을 제어하는 데 활용할 수 있습니다.

	// 코인 획득 기능 개발을 위해 우선 여기에서 선언. 추후에 코인 시스템이 완성되면 이 부분은 코인 시스템으로 이동하거나, 게임 상태와 코인 시스템이 서로 참조할 수 있도록 구조를 개선할 수 있습니다.
	_uint coin_count = 0; // 플레이어가 획득한 코인 수를 나타내는 변수. 필요에 따라 게임 내에서 코인 획득과 소비를 관리하는 데 활용할 수 있습니다.
