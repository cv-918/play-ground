...
#include "Scenes/OutGameLayoutStationScene.h"
...
#ifndef SHIPPING
		curr_scene_ = new OutGameLayoutStationScene();
		break;
#else
		_SYSTEM_LOG_ERROR(_T("OutGameLayoutStation scene is only available in debug builds."));
		next_scene_type_ = SceneType::Count;
		transition_phase_ = TransitionPhase::None;
		transition_elapsed_ = 0.0;
		transition_alpha_ = 0.f;
...
std::wstring SceneManager::_GetSceneName(SceneType _type) const
{
	...
	case SceneType::OutGameLayoutStation:	return L"OutGameLayoutStation";
	}
