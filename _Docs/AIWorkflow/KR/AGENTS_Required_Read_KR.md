...
`Initialize`, `OnEnter`, `OnExit`, `Ready`, `Load`, `Setup` 같은 함수에서 일부 Scene 상태를 만든 뒤 전체 생명주기를 중단하지 않는다. 단, 해당 함수가 원자적으로 실패하도록 설계된 경우는 예외다.

하위 기능 하나만 invalid라면 전체 초기화를 중단하지 말고 해당 하위 기능만 guard한다.
