...
| WF-021 | P2 | todo | maintenance | Harden Discord bot Node warnings and commandRunner shell usage | Bot logs show deprecation/security warnings that should be cleaned up after always-on operation is stable | Codex App -> manual validation | warning-free bot log check |
...
| GAME-005 | P2 | ready_for_implementation | refactoring | Replace `GetDataByIndex(0)` with deterministic character selection | `unordered_map` index access may be non-deterministic | Codex -> Copilot | approved: "Approve reduced-scope implementation: replace GetDataByIndex(0)-based playable character selection with explicit default character ID lookup. Do not add selected character save schema, character selection UI, unlock policy changes, or JsonDataManager refactor." |
