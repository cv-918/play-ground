# Folder Purpose: Handoff Role Workers

This folder stores contracts and templates for role chats or future role-worker automations that consume Handoff Queues.

Use this folder for:

- role worker intake rules
- queue consumption contracts
- intake decision templates
- role worker harness rules
- future low-risk worker automation boundaries

Do not use this folder for:

- Handoff Packets
- generated Queues
- generated Dashboard files
- game source code
- gameplay JSON
- automation scripts
- commit or push automation

The role worker documents here do not grant permission to edit source, JSON, assets, build settings, approval evidence, or Git history.
