...
승인된 범위 안에서 구현에 소스 코드 수정이나 스키마 변경이 아닌 데이터 수정이 필요하다는 이유만으로 Packet을 승인 대기에 넣지 않는다.

승인 대기는 아직 승인된 실행 범위가 없거나, 역할이 이미 승인된 범위 밖으로 나가야 할 때 필요하다.

승인된 실행 범위 안에서 필요한 일반적인 Developer 소스 수정과 스키마 변경이 아닌 데이터 수정은 실행의 일부다.
